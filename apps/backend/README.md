...
Create a `.env` file using the `.env.example` template at the root of the repository. This is shared
...
We use Alembic + SQLAlchemy for our database. This comes with the possiblity to auto-generate and
run database migration scripts. These scripts should typically be made after any change to the
database model before pushing the new code. Migrations can be created by calling the command:
